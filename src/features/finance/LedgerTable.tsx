...
export function LedgerTable({
  ...
  const groups = useMemo(() => {
    const map: Record<string, { key: string; date: Date; txns: Transaction[]; income: number; expense: number }> = {};
    for (const tx of transactions) {
      const d = new Date(tx.date);
      const key = format(d, 'yyyy-MM-dd');
      if (!map[key]) map[key] = { key, date: d, txns: [], income: 0, expense: 0 };
    ...
    return Object.values(map)
      ...
      .map((g) => ({ ...g, txns: [...g.txns].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) }));
  }, [transactions]);
  ...
  return (
    <>
      ...
      <div className="hidden md:block overflow-hidden rounded-lg border bg-card shadow-sm">
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {groups.map((g) => (
              <Fragment key={g.key}>
                <tr className="sticky top-[41px] z-[9] bg-secondary">
                  <td colSpan={batchMode ? 8 : 7} className="px-3 py-1.5 text-xs font-bold text-muted-foreground">
                    <div className="flex items-center justify-between">
                      <span>{format(g.date, 'M月d日 EEEE', { locale: zhCN })}</span>
                      <span className="font-mono text-[11px] font-medium">
                        <span className="text-success">{t('finance.income')} +{formatMoney(g.income)}</span>
                        <span className="mx-1 text-muted-foreground">·</span>
                        <span className="text-destructive">{t('finance.expense')} −{formatMoney(g.expense)}</span>
                      </span>
                    </div>
                  </td>
                </tr>
      ...
      {/* 手机：紧凑列表 */}
      <div className="md:hidden divide-y divide-border rounded-lg border bg-card shadow-sm">
        {groups.map((g) => (
          <Fragment key={g.key}>
            <div className="bg-secondary px-3 py-1 text-xs font-bold text-muted-foreground">
              <div className="flex items-center justify-between">
                <span>{format(g.date, 'M月d日', { locale: zhCN })}</span>
                <span className="font-mono text-[11px] font-medium">
                  <span className="text-success">+{formatMoney(g.income)}</span>
                  <span className="mx-1 text-muted-foreground">/</span>
                  <span className="text-destructive">−{formatMoney(g.expense)}</span>
                </span>
              </div>
            </div>
            {g.txns.map((tx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-muted-foreground">{cat?.name || t('finance.untitled')} · {accountText}</div>
                  </div>
