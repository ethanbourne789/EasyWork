...
> 版本：4.4.0 | 日期：2026-07-02 | 状态：**已审阅修订**
>
> **文档版本说明**：本文档（Design4）为最新版本，取代之前的 Design、Design2、Design3、Mail-Design3。
> Design3 中提到的"天气"功能和"Markdown 支持"已从本版本移除（YAGNI 原则），笔记模块统一使用 flutter_quill 富文本编辑器。
>
> **4.3.0 修订内容**：enough_mail API 保真度审计，修复 10 个 API 不匹配问题：
> - 修复 SearchQuery → MailSearch（SearchQuery 类不存在）
> - 修复 mailClient.search() → mailClient.searchMessages()
> - 修复 MessageBuilder.prepareReplyMessage → prepareReplyToMessage（方法名错误）
> - 修复 MessageBuilder.prepareForwardMessage 参数 fromAddress → from
> - 修复 MailAccount.fromManualSettings 参数名（imapHost→incomingHost 等）
> - 修复 MailAccount.fromDiscoveredSettings 缺少 userName 必填参数
> - 修复 MailAccount.fromManualSettingsWithAuth 参数（password→auth, 移除 loginType）
> - 修复 listMailboxesAsTree 返回类型 List<Mailbox> → Tree<Mailbox?>
> - 修复 MailClient.sendMessage 返回类型 Response → void
> - 修复 MimeMessageDownloader.fetchMessageContents 委托类型描述
>
> **4.2.0 修订内容**：经过 CheckRules.md 七维度审计，修复 77 个问题：
> - 补全缺失的类定义（EmailConnectionLostEvent、EmailConnectionReestablishedEvent、OnlineEvent）
> - 补全缺失的 Provider（taskListProvider、emailAccountListProvider、settingsDaoProvider 等）
> - 补全缺失的 DAO 方法（findByMessageId、insertMessage、updateMessage 等）
> - 修复跨章节矛盾（TaskStatus 枚举↔DB映射、EventBus时序、schema版本）
> - 完善数据流（离线重试策略、通知调度、错误反馈）
> - 补全架构完整性（缺失页面、路由、DAO分配）
> - 补全边界条件（空状态、离线行为、分页规范）
>
> **4.4.0 修订内容**：12项架构完整性补全：
> - 修复 mailDataSourcesProvider（Provider → StateNotifierProvider，含生命周期管理）
> - 修复 emailRepositoryProvider（添加初始化安全依赖说明）
> - 补全 CalendarEvents 表定义（schemaVersion 1→3，含 CalendarDao）
> - 补全股票模块完整设计（StockEntity/StockQuote、Sina Finance API、缓存、UI 页面）
> - 补全运动模块第三方同步设计（ExerciseSyncService 接口、SyncSource 枚举）
> - 补全 BackupService 完整实现（逐表导出/导入、版本校验、恢复前自动备份、旧备份清理）
> - 补全 Phase 0 详细检查清单（12项初始化步骤）
> - 补全 AttachmentStorageManager 工具类（下载、读取、删除、清理孤立附件、容量统计）
